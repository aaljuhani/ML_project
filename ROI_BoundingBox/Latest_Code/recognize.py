...
class recognize:
	...
	def predict(self, PRED_PATH , MODEL_PATH):
		...
		#hist = self.desc.describe(gray)
		#label_path = "./"
		des = self.sift.get_features(PRED_PATH)
		#des, self.TRAIN_LABELS = self.sift.extract(PRED_PATH)
		# print("******* generating sift histogram ********")
		sift_feat = self.sift.gen_histogram(des)
		sift_feat.reshape(1, -1)
		#des, train_labels = self.sift.get_features(imagePath, label_path)
		# print("******* generating sift histogram ********")
		#sift_feat = self.sift.gen_histogram(des)
		# print("******* predicting ********")
	...
	def predict(self, PRED_PATH , MODEL_PATH):
            model_clone = joblib.load(MODEL_PATH)        
            des, label = self.sift.get_features(PRED_PATH)
            #print("des==",des) 
     
            if des is None:
                return [0]      
            sift_feat = self.sift.gen_histogram(des)
            prediction = model_clone.predict(sift_feat.reshape(1, -1))
            return prediction
...
if __name__ == '__main__':
    ...
    '''
    data = "/Users/naveentr/Desktop/histosearch/predi_images"

    gt = "/Users/naveentr/Desktop/histosearch/mitoses_ground_truth"

    Model_path = "./sift_svm_20181204-095339.pkl" 

    Pred_path = "./naveen/02.png"
    '''
    data = "./fullrun_1"
    ...
    Model_path = "./sift_svm_20181204-095339.pkl" 
    #Model_path = model
    Pred_path = "./fullrun_1/TUPAC-TE-200.svs_01_02.png"

    mitoses_model = recognize(data, gt)
...
'''
if __name__ == '__main__':
    data = '../../DATA/mitosis_image_data/'
    gt = '../../DATA/mitoses_ground_truth/'
    Model_path = "logs/sift_svm_20181204-095339.pkl"
    Pred_path = "01.tif"
    mitoses_model = recognize(data, gt)
    mitoses_model.sift.MBkmeans = joblib.load('logs/kmeans_20181204-095339.pkl')
    predict_val = mitoses_model.predict(Pred_path, Model_path)
    print("predict val =", predict_val)
'''
