...
def draw_external_bound(positive_outputs):
    ...
    for tile in load_tiles:
        ...
        cur_tile_fn =  tile.filename.replace(images_folder_path,"")
        cur_tile_fn = cur_tile_fn[1:]
    ...
    pickle.dump(load_new_tiles, open("tiles_pickle_bound.sav", 'wb'), protocol=2)
...
def draw_bound(tile):
    ...
    rimg.show()
    rimg.save(tile.filename) #Saving the image with bounding box.
